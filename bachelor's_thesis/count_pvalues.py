...
def count_p_values(values, directory, treshold, column, headers):
    if values.skip!=None:
        ...
        for file in files:
            ...
            with open(root+'/'+file,'r') as f:
                ...
                for line in f:
                    ...
                    total_genes+=1
                    columns=line.split()
                    if float(columns[column])>=treshold:
                        p2+=1
                        if first:
...
def check_optparsing(values, optparser):
    ...
    if values.treshold_m!=None:
        values.treshold_m=values.treshold_m.split()
    if values.treshold_d!=None:
        values.treshold_d=values.treshold_d.split()
    if values.treshold_o!=None:
        values.treshold_o=values.treshold_o.split()
    if values.top!=None:
        assert values.muffinn_results!=None, "Give MUFFINN result file to get tresholds"
        assert values.dendrix_results!=None, "Give Dendrix result file to get tresholds"
        assert values.onco_results!=None, "Give Oncodrive-fm result file to get tresholds"
        assert os.path.isfile(values.muffinn_results), "Could not find file "+values.muffinn_results+" from directory"+os.getcwd()+'.\n'
        assert os.path.isfile(values.dendrix_results), "Could not find file "+values.dendrix_results+" from directory"+os.getcwd()+'.\n'
        assert os.path.isfile(values.onco_results), "Could not find file "+values.onco_results+" from directory"+os.getcwd()+'.\n'
        if values.treshold_m!=None:
        ...
        if values.treshold_d!=None:
            ...
            values.treshold_d=get_tresholds(values.dendrix_results, values.top, 0, 0)
        if values.treshold_o!=None:
            values.treshold_o+=get_tresholds(values.onco_results, values.top, 1, 4)
        else:
            values.treshold_o=get_tresholds(values.onco_results, values.top, 1, 4)
    return values
...
def optparsing():
    ...
    group.add_option("--treshold_m", dest="treshold_m", type="float", help="Treshold for muffinn outputs")
    group.add_option("--treshold_d", dest="treshold_d", type="float", help="Treshold for dendrix outputs")
    group.add_option("--treshold_o", dest="treshold_o", type="float", help="Treshold for oncodrive outputs")
    optparser.add_option_group(group)
...
def main():
    ...
    for treshold_m in values.treshold_m:
        muffinn_p1, muffinn_p2=count_p_values(values, values.muffinn, treshold_m, 9,1)
    ...
    for treshold_d in values.treshold_d:
        dendrix_p1, dendrix_p2=count_p_values(values, values.dendrix, treshold_d, 0, 0)
    ...
    for treshold_o in values.treshold_o:
        oncodrive_p1, oncodrive_p2=count_p_values(values, values.oncodrive, treshold_o, 1, 4)
