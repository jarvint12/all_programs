...
## Separates AA, hMDS, control etc. files from one another.
...
def move_files(values):
    ...
    i=0
    with open(values.infofile, 'r', encoding='utf-8') as f_r:
...
#                        os.system("mv "+root+'/'+dir+" /csc/mustjoki2/variant_move/epi_ski/mutation_load_tool/reports_permutation_HRUH/all_permutations/aa_samples/")
    ...
    for line in lines:
        ...
        if columns[1].strip()=="AA":
            for sample_id in aa_files:
                if sample_id in columns[0].strip():
                    # if "'HRUH1578_PB_CD8_1_16" in columns[0].strip():
                    #     print(sample_id, columns[0].strip())
                    found=True
                    break
            if not found:
                print(line)
